...
# --- Filtrage du corpus ---
# On exclut certains sites non pertinents pour cette analyse (funéraires, sites atypiques...)
# et on ne garde que le premier intervalle chronologique (-60/1)
d_filtered <- d_intervalle %>%
  filter(!site_abbreviation %in% c("STGRRFUN", "VSA", "ROM", "CLF")) %>%
  filter(intervalle_personnalise == "-60-1")
...
write.csv2(contingence,
           "resultats/contingence_urbain_premier_intervalle.csv",
           row.names = TRUE)
...
# "AFA" : site mis en individu supplémentaire (projeté sur les axes sans contribuer à leur calcul)
modalite_sup <- c("indéterminé", "huisserie charpente", "divers", "récipient culinaire")
row_sup_indices <- which(rownames(contingence) == "AFA")
indices_sup <- which(colnames(contingence) %in% modalite_sup)

# --- Calcul de l'AFC ---
# ncp = 10 : on conserve 10 dimensions
# row.sup / col.sup : lignes et colonnes projetées en supplémentaire
...
df_lignes <- data.frame(
  ...
  Coord_Axe3 = coord_lignes[, 4],   # NB : nom de colonne dupliqué (Axe4 nommé Axe3) — à corriger si besoin
  Cos2_Axe1 = cos2_lignes[, 1],
  Cos2_Axe2 = cos2_lignes[, 2],
  Cos2_Axe3 = cos2_lignes[, 3],
  Cos2_Axe3 = cos2_lignes[, 4],
  Contrib_Axe1 = contrib_lignes[, 1],
  Contrib_Axe2 = contrib_lignes[, 2],
  Contrib_Axe3 = contrib_lignes[, 3],
  Contrib_Axe3 = contrib_lignes[, 4]
)

write.csv2(df_lignes,
           "resultats/modalites_lignes_premier_intervalle.csv",
           row.names = TRUE)
...
write.csv2(contrib,
           "resultats/contrib_pourcentage_premier_intervalle.csv",
           row.names = TRUE)
...
# Ici, k est fixé manuellement à 3 (plutôt que de prendre automatiquement le maximum de silhouette),
# probablement pour des raisons d'interprétabilité archéologique de la partition
k_optimal <- 3
