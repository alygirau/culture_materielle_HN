...
# contexte urbain) et on ne garde que le troisième intervalle chronologique (100-300)
d_filtered <- d_intervalle %>%
  filter(!site_abbreviation == "STGRRFUN") %>%
  filter(intervalle_personnalise == "100-300")
...
write.csv2(contingence,
           "resultats/contingence_urbain_troisieme_intervalle.csv",
           row.names = TRUE)
...
# "AFA" : site mis en individu supplémentaire (comme pour le premier intervalle,
# mais contrairement au deuxième intervalle où aucun site n'était en row.sup)
...
result_afc <- CA(contingence, ncp = 10, col.sup = indices_sup, row.sup = row_sup_indices, graph = FALSE)
...
write.csv2(df_lignes,
           "resultats/modalites_lignes_troisieme_intervalle.csv",
           row.names = TRUE)
...
write.csv2(df_colonnes,
           "resultats/modalites_colonnes_troisieme_intervalle.csv",
           row.names = TRUE)
...
write.csv2(contrib,
           "resultats/contrib_pourcentage_troisieme_intervalle.csv",
           row.names = TRUE)
...
# Comme pour le deuxième intervalle, k est repris automatiquement du maximum
# de silhouette (contrairement au premier intervalle où il était fixé à 3)
