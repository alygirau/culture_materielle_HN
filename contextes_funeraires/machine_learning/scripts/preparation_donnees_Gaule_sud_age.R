...
# Zone géographique : Gaule du sud
...
# ============================================================
# Identique au script équivalent pour la Gaule du nord : catégories d'objets
# trop vagues, sépultures sans sexe étudié, classes d'âge non exploitables,
# et lignes sans âge précis renseigné (defunt_age).
...
# ============================================================
# Mêmes regroupements que dans les scripts équivalents (nord/sexe, nord/âge) :
# parure/vêtement, équipement de consommation alimentaire, récipient de
# stockage, armement, artisanat, soins du corps, équipement culturel.
...
#
# #Fusion des enfants (à commenter pour AFC)
# d_filtered <- d_filtered %>%
...
d_filtered <- d_filtered %>%
  mutate(defunt_classe_age = case_when(
    ...
    defunt_classe_age == "infans I ou II" ~ "infans I",
...
# ============================================================
# Bloc désactivé par défaut (voir note à la section 6), identique à celui du
# script équivalent pour la Gaule du nord.
...
# ============================================================
# NB : contrairement au script équivalent pour la Gaule du nord, ce script
# reconvertit également defunt_age en facteur à cette étape.
...
d_filtered$defunt_classe_age <- as.factor(d_filtered$defunt_classe_age)
d_filtered$depot_nom <- as.factor(d_filtered$depot_nom)
d_filtered$typeSep_nom <- as.factor(d_filtered$typeSep_nom)
d_filtered$defunt_age <- as.factor(d_filtered$defunt_age)
...
# ============================================================
d_filtered_gaule_sud <- d_filtered %>%
  filter(zone_geo == "Gaule du sud")


# ============================================================
# 13. CRÉATION DES INTERVALLES CHRONOLOGIQUES
# ============================================================
# Même logique que les scripts précédents : l'intervalle est calculé à partir
# du centre de la fourchette chronologique (moyenne entre phase_date_debut et
# phase_date_fin).
d_intervalle <- d_filtered_gaule_sud
