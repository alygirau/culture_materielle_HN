####
# Sujet : Gaule du sud — forêt aléatoire pour la prédiction du sexe du
...
# ============================================================
# Même logique que le script équivalent sur l'âge (voir ce script pour le
# détail). Les commentaires "CORRECTION" du script original ont été
# conservés tels quels : ils documentent des ajustements déjà appliqués par
# l'auteur (usage des prédictions OOF plutôt que des prédictions in-sample,
# metric = "ROC" cohérent avec summaryFunction = twoClassSummary).
train_random_forest <- function(X, y, title) {
  ...
  # NB : contrairement aux autres fonctions d'entraînement de ce jeu de
  # scripts, ce ggplot n'est pas enveloppé dans print() — à l'intérieur d'une
  # fonction, un objet ggplot n'est affiché à l'écran que s'il est
  # explicitement imprimé avec print(). Sans cela, ce graphique ne s'affichera
  # pas lors de l'exécution via train_random_forest(), contrairement aux
  # autres graphiques de cette fonction. Ajoute print() autour de l'appel
  # ci-dessous si tu veux que la courbe ROC s'affiche également.
  ggplot() +
